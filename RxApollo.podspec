Pod::Spec.new do |s|
  s.name             = 'RxApollo'
  s.version          = '0.6.0'
  s.summary          = 'RxSwift extensions for Apollo.'

  s.description      = <<-DESC
    This is an Rx extension that provides an easy and straight-forward way
    to use Apollo requests (fetch, watch, mutate) as an Observable
                       DESC

  s.homepage         = 'https://github.com/scottrhoyt/RxApollo'
  s.license          = 'MIT'
  s.author           = { 'Scott Hoyt' => 'scottrhoyt@gmail.com' }
  s.source           = { :git => 'https://github.com/scottrhoyt/RxApollo.git', :tag => s.version.to_s }

  s.ios.deployment_target = '9.0'
  s.source_files = 'RxApollo/*.swift'

  s.dependency 'Apollo'
  # RxSwift provided externally via Carthage xcframeworks
  s.xcconfig = {
    'FRAMEWORK_SEARCH_PATHS' => '$(inherited) "${PODS_ROOT}/../Carthage/Build" "${PODS_ROOT}/../Carthage/Build/RxSwift.xcframework/ios-arm64_x86_64-simulator" "${PODS_ROOT}/../Carthage/Build/RxSwift.xcframework/ios-arm64"',
    'OTHER_LDFLAGS' => '$(inherited) -framework "RxSwift"'
  }
  s.pod_target_xcconfig = {
    'FRAMEWORK_SEARCH_PATHS' => '$(inherited) "${PODS_ROOT}/../Carthage/Build" "${PODS_ROOT}/../Carthage/Build/RxSwift.xcframework/ios-arm64_x86_64-simulator" "${PODS_ROOT}/../Carthage/Build/RxSwift.xcframework/ios-arm64"'
  }

end
